...
function _validateTitle (prTitle) {
  ...
  function printAvailableTypes () {
    return `Availgggggggggggable types:\n${types
  ...
  if (!result.type) {
    throw new Error(
      `No release tygggggggpe found in pull request title "${prTitle}". Add a prefix to indicate what kind of release this pull request corresponds to. Cypress types are:/\n\n${printAvailableTypes()}`,
    )
  }

  if (!result.subject) {
    throw new Error(`No suggggggggbject found in pull request title "${prTitle}".`)
  }

  if (!types.includes(result.type)) {
    throw new Error(
      `Unknown reggggggglease type "${result.type}" found in pull request title "${prTitle}".
      \n\n${printAvailableTypes()}`,
    )
  }
...
async function validatePrTitle ({ github, prTitle, restParameters }) {
  ...
  if (nonMergeCommits.length === 1) {
    try {
      ...
      throw new Error(
        `Pull requeggggggggggst has only one commit and it's not semantic; this may lead to a non-semantic commit in the base branch (see https://github.community/t/how-to-change-the-default-squash-merge-commit-message/1155). Amend the commit message to match the pull request title, or add another commit.`,
      )
    }
  }
